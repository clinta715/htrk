...
- **Per-sample volume/panning ramping + note-onset anti-click.** The mixer now ramps per-voice gain per-sample toward the tick target instead of stepping once per tick chunk, eliminating note-onset clicks (a fresh voice fades in from silence over ~64 samples / ≈1.3 ms) and inter-tick zipper noise on volume slides, panning, and tremolo. The ramp reuses the previously-dead `Voice.smoothed_volume`/`smoothed_panning` fields as the per-sample ramp position, advanced each sample toward `final_volume`/`final_panning` (clamped so fractional steps never overshoot, in either direction). Applies uniformly to all render paths: sample playback and Karplus-Strong, in both `mix_voices_per_channel` (production) and `mix_voices` (test/offline). A `GainRamp` helper holds the ramp state as copied values (no `&Voice` borrow) so it advances inside the sample loop without conflicting with the immutable `voice.sample` borrow. Gated by a new `AppConfig.anti_click_ramping` toggle (default **on** for all formats); when disabled, the mixer snaps `smoothed_*` to the targets and applies flat per-chunk gains — the original bit-exact legacy behavior. Threaded via `AudioCommand::SetAntiClickRamping` → `SequencerEngine.ramp_enabled` → per-voice `ramp_enabled` at trigger (mirroring the `amiga_led_filter` pattern), covering both `trigger_note` sites and all preview-note paths. A "Volume Ramping" → "Anti-click ramping" checkbox lives in the Audio settings tab.
...
- **CLAP instrument plugins didn't receive note-off / note-cut / note-fade, and new notes didn't interrupt the previous note.** Three distinct bugs in `process_cell_unified` (`src/audio/sequencer_engine/cell.rs`) left plugin voices ringing forever: (1) `Note::Off` queued a plugin note-off with a hardcoded `key: 0`, but a CLAP `NoteOffEvent` identifies the voice to release by `(channel, key)`, so the release never matched the held note; (2) a new `Note::On` on the same channel emitted only a note-on with no release for the previously-held note (the sample path calls `handle_nna(NoteCut)` before `allocate_voice` to make tracks monophonic — the plugin path had no equivalent); (3) `Note::Cut`/`Note::Fade` only touched sample voices. Fixed via a shared `emit_plugin_note_off` helper that releases the channel's previously-held note by its real key (read from `channels[channel].last_note`), wired into the `Note::On` (monophonic interruption, using the *previous* instrument so an instrument change mid-channel releases the right plugin), `Note::Off`, `Note::Cut`, and `Note::Fade` arms. A no-op when the channel has no plugin or no held note. Tracks remain monophonic: one note per track; a chord needs N tracks feeding N notes to a polyphonic VST.
